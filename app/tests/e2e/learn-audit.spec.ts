...
/**
 * Learn-by-Playing rule audit. Plays a FULL "Learn by Playing" game (the real
 * First Reconnaissance engine, fixed seed) autonomously and, after every action,
 * checks rulebook invariants on window.__MK_STATE__. The guide is hidden so we
 * audit the underlying game (the guide is a non-interactive overlay that never
 * changes state). A clean run reaches the score screen with ZERO console errors,
 * crashes and rule violations.
 */
...
type Snapshot = {
  ...
  level: number; handLimit: number; unitLimit: number; unitCount: number
  crystals: { red: number; blue: number; green: number; white: number }
  diceCount: number; movePointsAvailable: number; movePointsSpent: number; handSize: number
}
...
async function readState(page: Page): Promise<Snapshot | null> {
  return await page.evaluate(() => {
    ...
    return {
      ...
      handLimit: p.handLimit as number, unitLimit: p.unitLimit as number,
      unitCount: (p.units as unknown[]).length, crystals: mana.crystals as Snapshot['crystals'],
...
function checkInvariants(s: Snapshot, expectedDice: number | null): string | null {
  ...
  if (expectedDice != null && s.diceCount !== expectedDice) return `Source dice count changed: ${s.diceCount} (expected ${expectedDice})`
  if (bad(s.movePointsAvailable) || s.movePointsAvailable < 0) return `movePointsAvailable negative/NaN: ${s.movePointsAvailable}`
  if (bad(s.movePointsSpent) || s.movePointsSpent < 0) return `movePointsSpent negative/NaN: ${s.movePointsSpent}`
  ...
  // Learn mode is the First Reconnaissance scenario: exactly 3 rounds.
  if (s.totalRounds !== 3) return `learn scenario should be 3 rounds, got ${s.totalRounds}`
  if (s.round < 1 || s.round > 3) return `round out of [1,3]: ${s.round}`
  return null
}
...
test.describe('Learn-by-Playing rule audit', () => {
  test.setTimeout(600_000)

  test('plays a full guided game with no rule violations', async ({ page }) => {
    const errors: string[] = []
    ...
    await page.goto('/?debug=1') // no ?seed → learn mode uses its fixed seed
    await page.evaluate(() => {
    ...
    // Hide the teaching guide so it can't sit over any control while the bot plays.
    const hideGuide = page.getByRole('button', { name: /Hide guide/i }).first()
    if (await visible(page, hideGuide, 4000)) await hideGuide.click({ force: true }).catch(() => undefined)
    ...
    const audit = async (step: number) => {
      ...
      const record = (v: string | null) => {
        if (!v) return
        const msg = `step ${step} (phase=${snap.phase}, round=${snap.round}): ${v}`
    ...
    for (let step = 0; step < 1200; step++) {
      ...
      if (stuckCounter > 50) {
        ...
        console.log(`[learn-audit] STUCK at step ${step}. phase=${snap?.phase} round=${snap?.round} fame=${snap?.fame}`)
        console.log(`[learn-audit] visible buttons: ${JSON.stringify(btns.slice(0, 30))}`)
        throw new Error(`[learn-audit] stuck at step ${step}`)
      }

      // Keep the guide out of the way if a reactive lesson reopened it.
      if (await visible(page, hideGuide, 0)) await hideGuide.click({ force: true }).catch(() => undefined)
      ...
      if (await visible(page, tacticOverlay)) {
        ...
        for (let c = 0; c < n; c++) {
          ...
          await cards.nth(c).click({ force: true }).catch(() => undefined); picked = true; break
        }
    ...
    console.log(`[learn-audit] turnsEnded=${turnsEnded}, reachedScore=${reachedScore}, violations=${violations.length}, errors=${errors.length}`)
